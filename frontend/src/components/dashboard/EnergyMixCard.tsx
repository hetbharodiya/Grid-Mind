...
const CustomMixTooltip: React.FC<CustomTooltipProps> = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-lg text-sm text-gray-900 space-y-1.5 min-w-[170px]">
...
export const EnergyMixCard: React.FC<EnergyMixCardProps> = ({ summary }) => {
  const mixData = useMemo(() => {
    ...
    return rawItems.map((item) => ({
      ...item,
      percentage: summary.total_supplied_energy_kwh > 0
        ? Math.max(0, (item.value / summary.total_supplied_energy_kwh) * 100)
        : 0,
    }));
  }, [summary]);
  ...
  return (
    <SectionCard
      ...
      badge={
        <span className="text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded bg-sky-50 text-sky-700 border border-sky-200 font-mono">
          Source Share
        </span>
      }
    >
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center min-h-[224px]">
        {/* Donut Chart */}
        <div className="sm:col-span-5 h-56 relative flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-500 font-mono">Clean</span>
          ...
          {mixData.map((item) => (
            ...
            >
              ...
              <div className="flex items-center space-x-3 font-mono">
                <span className="text-gray-600 text-xs md:text-[13px]">
                  {item.value.toFixed(1)} kWh
                </span>
                <span className="w-12 text-right font-bold text-gray-900 text-xs md:text-sm">
